...
public class PollingProgressDialog extends ProgressDialog implements ProgressObserver, OnCancelListener {
    private final Context _context;
    private final Handler _handler;
    private final ProgressSource _src;
    private int _min;
    ...
    public PollingProgressDialog(Context context, ProgressSource src,
                                 String msg, boolean showProgress, boolean cancellable) {
        super(context);
        _src = src;
        _context = context;
        if (msg == null) {
            msg = _context.getString(R.string.progress_dialog_message);
        }
        ...
        if (cancellable) {
            setButton(BUTTON_NEGATIVE, _context.getString(android.R.string.cancel), (dialog, which) -> dialog.cancel());
