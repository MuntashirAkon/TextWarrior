...
SharedPreferences.OnSharedPreferenceChangeListener{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if(savedInstanceState == null){
			...
			/*
			_recoveredUiState = (UiState) savedInstanceState.getParcelable(STATE_UI);
			//XXX if worker threads were interrupted, display info box
			
			int recoveryFileType = getRecoveryFileType();
			if(recoveryFileType == RECOVERY_FILE_TEMP){
				//XXX handle accessing of temp file by multiple instances of the activity
				open(TEMP_FILE_PATH);
			}
			else if(recoveryFileType == RECOVERY_FILE_SOURCE){
				SharedPreferences recoveryFilePrefs = getSharedPreferences(PREFS_RECOVERY_FILE, 0);
				String sourceFile = recoveryFilePrefs.getString(RECOVERY_STATE_SOURCE_FILE, "");
				open(sourceFile);
			}
			else if(recoveryFileType == RECOVERY_FILE_CREATE_ERROR){
				//XXX inform user that edits could not be recovered
				resetRecoveryFilePrefs(); //clear recovery error state
			}
			*/
		}
	...
	private void analyzeTextProperties() {
		...
		PollingProgressDialog dialog =
			new PollingProgressDialog(this, _taskAnalyze, true, true);
	...
	public void find(String what, boolean isCaseSensitive, boolean isWholeWord){
		if(what.length() > 0){
			...
			PollingProgressDialog dialog = new PollingProgressDialog(this, _taskFind, false, false);
			dialog.startDelayedPollingDialog();
			_taskFind.start();
		}
	}
	...
	public void replaceAll(String what, String replacementText,
		...
		if(what.length() > 0){
			...
			PollingProgressDialog dialog = new PollingProgressDialog(this, _taskFind, true, false);
			dialog.startDelayedPollingDialog();
			_taskFind.start();
		}
	}
	...
	public void open(String filename){
		...
		PollingProgressDialog dialog = new PollingProgressDialog(this, _taskRead, true, true);
		dialog.startDelayedPollingDialog();
		_taskRead.start();
	}
	...
	public void save(String filename, boolean overwrite){
		...
		PollingProgressDialog dialog = new PollingProgressDialog(this, _taskWrite, true, true);
		dialog.startDelayedPollingDialog();
		_taskWrite.start();
	}
	...
	/*
	 ...
	 // onPause(). However, this is problematic because saving of files can take
	 // an arbitrarily long time, and onPause() is supposed to be lightweight.
	 // Temp file saving can be done in onDestroy(), but this means that if the 
	 // system decides to kill the activity's process, all edits will be lost.
	protected void onPause(){
		...same as existing code...
		
		//save edits in a temp file if process is being force-killed to reclaim memory
		if(!isFinishing() && !configurationChange) {
			//XXX Associate temp file metadata with a particular activity instance.
			// Example bug: Activity has 2 running instances A and B. A is force-
			// killed and metadata is saved in PREFS_RECOVERY_FILE. B is then force-
			// killed and overwrites the PREFS_RECOVERY_FILE. When A is run again,
			// it wrongly uses the metadata saved by B.
			SharedPreferences settings = getSharedPreferences(PREFS_RECOVERY_FILE, 0); 
			SharedPreferences.Editor editor = settings.edit();
			editor.putInt(RECOVERY_STATE_INTERRUPTED_TASKS, stopAllWorkerThreads());
			String fileToReload = _filename;
			boolean isDirty = _editField.isDirty();
	
			if(isDirty && !saveToRescueFile()){
				fileToReload = ""; //abandon unsaved edits
			}
	...
	private boolean saveToRescueFile(){
		boolean saveSuccess = false;
		
		String state = Environment.getExternalStorageState();
		//TODO 1: test if system will kill the activity before saving a large file is done
		//TODO 2: handle accessing of temp file by multiple instances of the activity
	    if (Environment.MEDIA_MOUNTED.equals(state)) {
	    	...
	    	try{
	    		FileOutputStream fs = new FileOutputStream(new File(tempFilePath));
	...
	private void resetRecoveryFilePrefs() {
		SharedPreferences.Editor editor = getSharedPreferences(PREFS_RECOVERY_FILE, 0).edit();
		editor.putString(RECOVERY_STATE_SOURCE_FILE, "");
		editor.putBoolean(RECOVERY_STATE_DIRTY, false);
		editor.putInt(RECOVERY_STATE_INTERRUPTED_TASKS, 0);
		editor.commit();
	}
	
	private int getRecoveryFileType() {
		SharedPreferences recoveryFilePrefs = getSharedPreferences(PREFS_RECOVERY_FILE, 0);
		String recoveryFile = recoveryFilePrefs.getString(RECOVERY_STATE_SOURCE_FILE, "");
		boolean isDirty = recoveryFilePrefs.getBoolean(RECOVERY_STATE_DIRTY, false);
		
		int type;
		if(recoveryFile.length() > 0){
			if(isDirty){
				type = RECOVERY_FILE_TEMP;
			}
			else{
				type = RECOVERY_FILE_SOURCE;
			}
		}
		else if(isDirty){
			type = RECOVERY_FILE_CREATE_ERROR;
		}
		else{
			type = RECOVERY_FILE_NONE;
		}
	...
	private static final int RECOVERY_FILE_NONE = 0;
	private static final int RECOVERY_FILE_SOURCE = 1;
	private static final int RECOVERY_FILE_TEMP = 2;
	private static final int RECOVERY_FILE_CREATE_ERROR = 4;
	private static final int RECOVERY_FILE_MISSING = 8;
	
	private final static String PREFS_RECOVERY_FILE = "recoveryFilePrefs";
	private final static String RECOVERY_STATE_DIRTY = "dirty";
	private final static String RECOVERY_STATE_SOURCE_FILE = "filename";
	private final static String RECOVERY_STATE_INTERRUPTED_TASKS = "interruptedTasks";
	
	private static final String TEMP_FILE_PATH = "/Android/data/com.myopicmobile.textwarrior.android/files/temp";
	*/
		...
		if(_taskRead != null){
			...
			if(_taskAnalyze.isDone()){
				...
				if(_taskFind.getRequestCode() == ProgressSource.REPLACE_ALL){
					dialog = new PollingProgressDialog(this, _taskFind, true, false);
				}
				else{
					dialog = new PollingProgressDialog(this, _taskFind, false, false);
				}
				dialog.startPollingDialog();
			}
		}
			...
			if(_taskAnalyze.isDone()){
				...
				_taskAnalyze.registerObserver(this);
				PollingProgressDialog dialog = new PollingProgressDialog(this, _taskAnalyze, true, true);
				dialog.startPollingDialog();
			}
		}
	}
