...
public class FindThread extends Thread implements ProgressSource{
	...
	public void run(){
		FindResults results = new FindResults(_searchText.length());
		
		switch(_requestCode){
		case ProgressSource.FIND:
			results.foundOffset = FINDER.wrappedFind(
					_src,
					_searchText,
					_start,
					_isCaseSensitive,
		...
		case ProgressSource.FIND_BACKWARDS:
			results.foundOffset = FINDER.wrappedFindBackwards(
					_src,
					_searchText,
					_start,
					_isCaseSensitive,
		...
		case ProgressSource.REPLACE_ALL:
			Pair replaceResult = FINDER.replaceAll(
					...
					_isCaseSensitive,
					_isWholeWord);
			results.replacementCount = replaceResult.getFirst();
			results.newStartPosition = replaceResult.getSecond();
	    	notifyComplete(results);
			break;
		default:
	...
	public final void forceStop(){
		// not stoppable or cancelable
	}
	
	// unused; search operations are not cancelable
